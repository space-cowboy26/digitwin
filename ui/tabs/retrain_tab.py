...
from ui.workers.retrain_worker import RetrainWorker
...
class RetrainTab(QWidget):
    ...
    def _build_ui(self):
        ...
        # title
        title = QLabel("🔄 Retrain Models — All Inverters")
        title.setStyleSheet("font-size: 18px; font-weight: 600; color: #1F2937;")
        layout.addWidget(title)

        # description
        desc = QLabel("Upload new Inverter and WMS Report files to fine-tune existing models.")
        ...
        # retrain button
        self.btn_retrain = QPushButton("Retrain All")
        self.btn_retrain.clicked.connect(self._on_retrain_clicked)
        self.btn_retrain.setMinimumHeight(40)
        layout.addWidget(self.btn_retrain)

        # progress bar — hidden until retraining starts
        ...
        # results area — populated after retraining
    ...
    def _on_retrain_clicked(self):
        """Called when operator clicks retrain All button."""
        ...
        # Note: run_batch_retrain doesn't have overwrite parameter
        ...
        # "when worker fires finished signal, call self._on_retrain_complete"
        self.worker.finished.connect(self._on_retrain_complete)

        # "when worker fires error signal, call self._on_retrain_error"
        self.worker.error.connect(self._on_retrain_error)

        # start the worker — this calls worker.run() on a background thread
        self.worker.start()

    def _on_retrain_complete(self, results: dict):
        """
        Called automatically when worker.finished signal fires.
        results is the dict returned by run_batch_retrain.
        This runs on the MAIN thread — safe to touch UI here.
        """
        # hide progress, re-enable button
        self.progress_bar.hide()
        self.btn_retrain.setEnabled(True)
        self.btn_retrain.setText("Retrain All")
        ...
        # show metrics table for retrained inverters
        if passed:
            table = MetricsTable(passed)
            self.results_layout.addWidget(table)

            # show plots for first retrained inverter
    ...
    def _on_retrain_error(self, error_msg: str):
        """Called if the worker crashes entirely."""
        self.progress_bar.hide()
        self.btn_retrain.setEnabled(True)
        self.btn_retrain.setText("Retrain All")
        self._show_error(f"Retraining failed: {error_msg}")
